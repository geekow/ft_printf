...
/*   Updated: 2017/01/29 17:00:12 by jjacobi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	flag_zero(char flag_minus, char flag_zero, int min_size, int i)
{
	if (!flag_minus && flag_zero && ((min_size - i) > 0))
		return (addchar('0', min_size - i));
	return (1);
}
...
int			parse_s(t_info *info, va_list args)
{
	...
	if (!flag_zero(info->flag_minus, info->flag_zero, info->min_size, i))
		return (-1);
	if (!flag_space(info->flag_minus, info->flag_zero, info->min_size, i))
		return (-1);
	if (info->lenght_modifs[0] == 'l' && info->lenght_modifs[1] == '\0')
	{
		i = 0;
